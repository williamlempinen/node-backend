...
router.post('/add-user-to-group', (0, validator_1.validator)(schema_1.default.addToGroup), (0, asyncHandler_1.asyncHandler)((request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
    Logger_1.default.info(`Request: ${JSON.stringify(request.body)}`);
    const { userId, conversationId } = request.body;
    const [conversation, error] = yield ConversationRepo_1.default.addUserToGroup(userId, conversationId);
    if (error)
        return next({ type: error.type, message: error.errorMessage });
    Logger_1.default.info(`Added user to group`);
    return (0, responses_1.SuccessResponse)('Added user to group', response, conversation);
})));
